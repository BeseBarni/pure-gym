...
## Context
The React client needs to know when a door has actually opened or if a hardware error occurred. Since the hardware communication is asynchronous (via RabbitMQ), the API cannot return a standard HTTP 200 OK with the final result immediately.

## Decision
We will use **Server-Sent Events (SSE)** for pushing state changes from the API to the Client.

1.  Client sends HTTP POST to request cabinet.
...
4.  The API pushes the state change (e.g., `CabinetLocked`, `AccessDenied`) to the specific client via an established SSE connection.
