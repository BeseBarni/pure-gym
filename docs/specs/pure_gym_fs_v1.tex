...
\begin{description}[style=multiline, leftmargin=3cm, font=\bfseries]
    ...
    \item[ACS-02] \textbf{Entry Validation.} When the QR is scanned at the \texttt{EnterGym} endpoint:
    \begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{API:} Validate OTP (RAM) $\rightarrow$ Send out \texttt{GymEntered} event $\rightarrow$ Return OK $\rightarrow$ \textbf{Hardware Opens Door}.
